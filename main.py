...
Usage:
    python main.py
    python main.py --transaction path/to/train_transaction.csv --identity path/to/train_identity.csv
...
def main() -> None:
    ...
    if not Path(Config.TRANSACTION_CSV).exists():
        ...
        logger.error(
            "Set via --transaction flag or FRAUD_TRANSACTION_CSV environment variable."
        )
        sys.exit(1)
    ...
    logger.info(
        "Launch dashboard: streamlit run dashboard.py -- --output %s", Config.OUTPUT_DIR
    )
